...
namespace Microsoft.FeatureFlighting.Core.Commands
{
    ...
    /// </summary>
    internal class ActivateStageCommandHandler: CommandHandler<ActivateStageCommand, IdCommandResult>
    {
        ...
        public ActivateStageCommandHandler(ITenantConfigurationProvider tenantConfigurationProvider,
        ...
        protected override async Task<IdCommandResult> ProcessRequest(ActivateStageCommand command)
        {
        ...
        private async Task<FeatureFlightAggregateRoot> GetFeatureFlight(ActivateStageCommand command, TenantConfiguration tenantConfiguration)
        {
            ...
            if (flightDto == null)
                throw new DomainException($"Flight for feature {command.FeatureName} does not existing for {tenantConfiguration.Name} in {command.Environment}",
                    "UPDATE_FLAG_001", command.CorrelationId, command.TransactionId, "ActivateStageCommandHandler:GetFeatureFlight");
