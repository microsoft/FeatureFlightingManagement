...
namespace Microsoft.FeatureFlighting.Core
{
    ...
    public class FeatureFlagEvaluator : IFeatureFlagEvaluator
    {
        ...
        public async Task<IDictionary<string, bool>> Evaluate(string applicationName, string environment, List<string> featureFlags)
        {
            ...
            @event.AddProperty("Batches", featureFlagBatches.Count());

            IDictionary<string, bool> result = await EvaluateFeatureFlagsInBatch(featureFlagBatches, tenantConfiguration, environment, @event);
            LogEvaluationResults(performanceContext, @event);
            return result;
        }

        private async Task<IDictionary<string, bool>> EvaluateFeatureFlagsInBatch(IEnumerable<IGrouping<int, string>> featureFlagGroups, TenantConfiguration tenantConfiguration, string environment, EventContext @event)
        {
