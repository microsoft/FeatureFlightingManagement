...
namespace Microsoft.FeatureFlighting.Core.Operators
{
    ...
    /// </summary>
    public class NotArrayAnyOperator : BaseOperator
    {
        public override Operator Operator => Operator.NotArrayAny;
        ...
        protected override Task<EvaluationResult> Process(string configuredValue, string contextValue, string filterType, LoggerTrackingIds trackingIds)
        {
            ...
            return Task.FromResult(new EvaluationResult(!contextValues.Any(value => configuredValues.Any(cxtValue => cxtValue.ToLowerInvariant() == value.ToLowerInvariant())), Operator, filterType));
        }
    }
}
