    ...
    public abstract class BaseFilter
    {
        ...
        public async Task<bool> EvaluateFlightingContextAsync(FeatureFilterEvaluationContext featureFlag, string defaultFilterKey = null)
        {
            ...
            try
            {
                ...
                var settings = GetFilterSettings(featureFlag);
                var filterKey = !string.IsNullOrWhiteSpace(settings.FlightContextKey)
                    ? settings.FlightContextKey.ToUpperInvariant()
                    : defaultFilterKey.ToUpperInvariant();

                var contextParams = GetFlightContext(filterKey, trackingIds);
                ...
                var op = (Operator)Enum.Parse(typeof(Operator), settings.Operator, true);
                contextValue = (contextParams[filterKey]!=null)? contextParams[filterKey].ToString().ToLowerInvariant() : string.Empty;
                var settingsValue = settings.Value.ToLowerInvariant();
                var evaluator = _evaluatorStrategy.Get(op);

                if (evaluator == null)
                    throw new Exception($"No evaluator has been assigned for operator - {Enum.GetName(typeof(Operator), op)}");

                var evaluationResult = await evaluator.Evaluate(settingsValue, contextValue, FilterType, trackingIds);
